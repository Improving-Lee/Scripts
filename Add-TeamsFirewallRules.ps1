...
.DESCRIPTION
    ...
    This script adds the necessary firewall rules for the that will account for the Teams executable in the user
...
.NOTES
    ...
    1.2     05-12-20 Updated to change error handling for Intune.
#>
...
If (Test-Path $profPath) {
    ...
    If (Get-NetFirewallApplicationFilter -Program $profPath -ErrorAction SilentlyContinue) {
        ...
        Try {
            ...
            Throw "Failed to remove existing rule(s) for $profPath."
        }
    } Else {
    ...
    Try {
        ...
        $status = $false
        Throw "Failed to create new firewall rules for $profPath."
    }
} Else {
...
If (Test-Path $progPath) {
    ...
    If (Get-NetFirewallApplicationFilter -Program $progPath -ErrorAction SilentlyContinue) {
        ...
        Try {
            ...
            Write-Log -Message "New rules may not apply as expected." -Severity Warning
            Throw "Failed to remove existing rule(s) for $progPath."
        }
    } Else {
    ...
    Try {
        ...
        $status = $false
        Throw "Failed to create new firewall rules for $progPath."
    }
} Else {
    Write-Log -Message "Teams executable not found for $progPath." -Severity Warning
}
